...
def update_map_window(entries):
    ...
    for i, entry in enumerate(entries):
        if i % 3 == 0:
            ...
            y_values.append(float(entry.get()))
        elif i % 3 == 2:
            z_values.append(float(entry.get()))

    fig = go.Figure(data=[go.Scatter3d(x=x_values, y=y_values, z=z_values, mode='markers')])

    fig.update_layout(title='Coordinates', autosize=True,
...
def create_map_window():
    ...
    entry_window = webview.create_window('Map', url='map.html', width=1600, height=900)
